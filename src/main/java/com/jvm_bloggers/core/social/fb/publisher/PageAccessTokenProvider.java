...
class PageAccessTokenProvider {
    ...
    @Autowired
    PageAccessTokenProvider(FacebookProperties facebookProperties, ObjectMapper mapper) {
    ...
    public String getToken() {
        ...
        final String
            appSecretProof =
            facebookClient.obtainAppSecretProof(facebookProperties.getUserToken(),
    ...
    private WebRequestor.Response queryForPageToken(String appSecretProof) throws IOException {
        return facebookClient.getWebRequestor().executeGet(
            String.format(PAGE_TOKEN_URL, facebookProperties.getPageId(),
                facebookProperties.getUserToken(), appSecretProof)
        );
    }
